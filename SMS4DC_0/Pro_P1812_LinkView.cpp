// Pro_P1812_LinkView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_P1812_LinkView, CFormView)

CPro_P1812_LinkView::CPro_P1812_LinkView()
	: CFormView(CPro_P1812_LinkView::IDD)
{
	//{{AFX_DATA_INIT(CPro_P1812_LinkView)
	m_Gr = 0.0;
	m_Gt = 0.0;
	m_Hr = 0.0;
	m_Ht = 0.0;
...
void CPro_P1812_LinkView::DoDataExchange(CDataExchange* pDX)
{
	CFormView::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CPro_P1812_LinkView)
	...
	DDX_Text(pDX, IDC_EDIT_Loss1546, m_Loss1812);
...
BEGIN_MESSAGE_MAP(CPro_P1812_LinkView, CFormView)
	//{{AFX_MSG_MAP(CPro_P1812_LinkView)
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1812_LinkView diagnostics

#ifdef _DEBUG
void CPro_P1812_LinkView::AssertValid() const
{
	CFormView::AssertValid();
}

void CPro_P1812_LinkView::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1812_LinkView message handlers


void CPro_P1812_LinkView::Plot(CDC* pDC,double *xi, double *yi,int N,
...
void CPro_P1812_LinkView::DrawAntSymbol(CDC* pDC,double xi, double yi,
						double xiMIN,double xiMAX,double yiMIN,double yiMAX,
...
void CPro_P1812_LinkView::TextDraw(CDC* pDC,int x, int y,CString str,COLORREF FontColor,
							int Escapement,
...
void CPro_P1812_LinkView::LineDraw(CDC* pDC,int x1, int y1,int x2, int y2,
...
void CPro_P1812_LinkView::DEG2DMS(CString type,double x,int *xD,int *xM,double *xS,char *xU) 
{
	if (x < 0)
	{
		x=-x;
...
void CPro_P1812_LinkView::YtickL(CDC* pDC,double yiMIN,double yiMAX) 
{
...
void CPro_P1812_LinkView::Xtick(CDC* pDC,double xiMIN,double xiMAX) 
{
	CPro_P1812_LinkDoc* pDoc = (CPro_P1812_LinkDoc *)GetDocument();
	CString label;
...
double CPro_P1812_LinkView::MinValue(double *xi,int N) 
{
...
double CPro_P1812_LinkView::MaxValue(double *xi,int N) 
{
...
double CPro_P1812_LinkView::InterpXY(double *D0,double *E0,double d,int num)
{
...
double CPro_P1812_LinkView::atof_kfactor(CString k1) 
{
	double k2 = 1.0;
	k1.Remove(' ');
	int n = k1.Find('/');
...
void CPro_P1812_LinkView::OnDraw(CDC* pDC) 
{
	CPro_P1812_LinkDoc* pDoc = (CPro_P1812_LinkDoc *)GetDocument();
...
void CPro_P1812_LinkView::InitForm(CPro_P1812_LinkDoc* pDoc) 
{
	UpdateData();
	m_Hr = pDoc->LinkRxH;
...
*/
	...
	GetDlgItem(IDC_EDIT_heff)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_STATICb4)->ShowWindow(SW_HIDE);

	UpdateData(false);
}


void CPro_P1812_LinkView::TxRxObj(CDC* pDC) 
{
...
//	TextDraw(pDC,x1+len1/4,y1+0.8*len1,"Tx",RGB(255,255,255),0,m_fontH100,0,FW_BOLD);
...
//	TextDraw(pDC,a+x1+len1/4,y1+0.8*len1,"Rx",RGB(255,255,255),0,m_fontH100,0,FW_BOLD);
...
void CPro_P1812_LinkView::DrawRect(CDC* pDC,int x1,int y1,int x2,int y2,COLORREF flColor) 
{
	CBrush brush(flColor);
...
void CPro_P1812_LinkView::DrawRoundRect(CDC* pDC,int x1,int y1,int x2,int y2,COLORREF flColor) 
{
	CBrush brush(flColor);
	CBrush *oldbrush = pDC->SelectObject(&brush);
...
void CPro_P1812_LinkView::OnBUTTONLink() 
{
	UpdateData();
...
void CPro_P1812_LinkView::OnRadioLR() 
{
	UpdateData();
...
void CPro_P1812_LinkView::OnRadioRL() 
{
	OnRadioLR();
}

void CPro_P1812_LinkView::OnInitialUpdate() 
{
	CFormView::OnInitialUpdate();

//	initDP_LP();

	CString Title_En  = "Link Calculation (P1812)";
	...
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_Loss1546);
	xx->SetFont(&font);
	...
	UpdateData(false);
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ChangeMenuLang(IDR_SMS4DCTYPE48);
	((CSMS4DCApp *)AfxGetApp())->TranslateDialog((CDialog *)this);
}

void CPro_P1812_LinkView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	if(pInfo)
...
*/
	...
	str.Format(_Z("Loss1812(dB) : ")+"%g",m_Loss1812);
...
void CPro_P1812_LinkView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
}
void CPro_P1812_LinkView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/) 
{
}
BOOL CPro_P1812_LinkView::OnPreparePrinting(CPrintInfo* pInfo) 
{
	((CSMS4DCApp*)AfxGetApp())->SetLandscape();
	return DoPreparePrinting(pInfo);
}

HBRUSH CPro_P1812_LinkView::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	if (  pWnd->GetDlgCtrlID() == IDC_EDIT_Distance		||
		  ...
		  pWnd->GetDlgCtrlID() == IDC_EDIT_Loss1546		||
...
void CPro_P1812_LinkView::OnChangeEDITSw() 
{
	UpdateData();
	m_S = m_Sw;
	UpdateData(false);
...
void CPro_P1812_LinkView::OnChangeEDITVSWR2w() 
{
	UpdateData();
	m_VSWR2 = m_VSWR2w;
	UpdateData(false);
...
void CPro_P1812_LinkView::OnFilePrintPreview() 
{
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarCheck();
	CFormView::OnFilePrintPreview();
}

void CPro_P1812_LinkView::OnEndPrintPreview(CDC* pDC, CPrintInfo* pInfo, POINT point, CPreviewView* pView) 
{
...
int CPro_P1812_LinkView::DP_LP(CDC* pDC,int nH_DP) //97/10/12
{
...
void CPro_P1812_LinkView::initDP_LP(CDC * pDC) //97/10/12
{
//	CDC * pDC = GetDC();
//	pDC->SetMapMode(MM_LOMETRIC);
