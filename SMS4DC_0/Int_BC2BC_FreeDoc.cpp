// Int_BC2BC_FreeDoc.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CInt_BC2BC_FreeDoc, CDocument)

CInt_BC2BC_FreeDoc::CInt_BC2BC_FreeDoc()
{
	colormaptype="Jet";
...
BOOL CInt_BC2BC_FreeDoc::OnNewDocument()
{
...
BEGIN_MESSAGE_MAP(CInt_BC2BC_FreeDoc, CDocument)
	//{{AFX_MSG_MAP(CInt_BC2BC_FreeDoc)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CInt_BC2BC_FreeDoc diagnostics

#ifdef _DEBUG
void CInt_BC2BC_FreeDoc::AssertValid() const
{
	CDocument::AssertValid();
}

void CInt_BC2BC_FreeDoc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CInt_BC2BC_FreeDoc serialization

void CInt_BC2BC_FreeDoc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
...
/////////////////////////////////////////////////////////////////////////////
// CInt_BC2BC_FreeDoc commands



void CInt_BC2BC_FreeDoc::GetData()
{
	...
	long AntIDY  ;	 AntIDY  = atol(GetFld(m_SelX,30));
...
double StepRes = min(disttest1, disttest2)/m_ZoomFactor;
	...
	for(int Yi = 0;Yi<Nrow;Yi++)
	{
		...
		for( i=0;i<37;i++)
		{
			if     (polarY == "H")		erpdbwY = erp_h_dbwY - attH0Y[i];
			else if(polarY == "V")		erpdbwY = erp_v_dbwY - attV0Y[i];
	...
	//		dist = max(dist,0.00001);
			...
			for(Yi=0;Yi<Nrow;Yi++)
			{
				...
				PR = IntFunc.BC_ProtectionR(freq_assgnX,freq_assgnY[Yi] , Xsys , Esi0,Esi0); 
...
double CInt_BC2BC_FreeDoc::Dist_km_Az_Deg(double lat1,double lon1,double lat2,double lon2,double *RAZIM) 
{
...
double CInt_BC2BC_FreeDoc::Azimuth_Deg(double lat1,double lon1,double lat2,double lon2) 
{
	double pi=4.0*atan(1.0);
	float	lat10 = (float)(lat1*pi/180.0),
			lon10 = (float)(lon1*pi/180.0),
...
double CInt_BC2BC_FreeDoc::Distance_km(double lat1,double lon1,double lat2,double lon2) 
{
	double pi=4.0*atan(1.0);
	float	lat10 = (float)(lat1*pi/180.0),
			lon10 = (float)(lon1*pi/180.0),
...
CString CInt_BC2BC_FreeDoc::GetFld(CString Src,int N)
{
	CString Fld = "";
	int place=0,Cnt=0;
...
void CInt_BC2BC_FreeDoc::GE84pattern(long terrakey,double * attnH,double * attnV,CString pol) 
{
...
double CInt_BC2BC_FreeDoc::Interp2(int *D0,double *E0,double d,int num) 
{
...
BOOL CInt_BC2BC_FreeDoc::PeekAndPump()
{
	static MSG msg;
