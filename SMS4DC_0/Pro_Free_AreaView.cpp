// Pro_Free_AreaView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_Free_AreaView, CView)

CPro_Free_AreaView::CPro_Free_AreaView()
{
	old_cx=old_cy=600;
	new_cx=new_cy=600;
...
BEGIN_MESSAGE_MAP(CPro_Free_AreaView, CView)
	//{{AFX_MSG_MAP(CPro_Free_AreaView)
	...
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_COMMAND(ID_TOOLS_CONTOURVALUE, OnToolsContourvalue)
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_AreaView drawing

#include "memdc.h"
void CPro_Free_AreaView::OnDraw(CDC* pDC)	//98/01/25
{
	CMemDC mDC(pDC);
	OnDraw4(mDC);
}
void CPro_Free_AreaView::OnDraw4(CDC* pDC)	//98/01/25
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
	...
	////////////////////////////////////////
		...
		double MinValue1 = max(pDoc->ErMin, pDoc->m_MinValue);
		double MaxValue1 = min(pDoc->ErMax, pDoc->m_MaxValue);
		if (pDoc->m_bThreshold)
		{
			MinValue1 = pDoc->ErMin;
			MaxValue1 = pDoc->ErMax;
		}
			...
			for (int j=0;j<H;j++)
				...
				//	{
						...
						else
						{
							if (pDoc->m_bThreshold)
							{
								if (min(max((*temp),MinValue1),MaxValue1) >= pDoc->m_Threshold)
								{	
									*temp2 = 159;
									m_CoveredPix++;
								}
								...
								*temp2 = Contour( min(max((*temp),MinValue1),MaxValue1) ,MinValue1,MaxValue1,pDoc->m_level);
							//	*temp2=(BYTE )(( min(max((*temp),MinValue1),MaxValue1) - MinValue1) * 255/(MaxValue1-MinValue1));
						}
				//	}
				//	else	*temp2=255;
...
//		fclose(fp);
		...
		*/
		...
		if (pbmfh)
		{
			...
			///////////////////////////////////////
		//	pDC->StretchBlt(0,0,new_cx,new_cy,&MemDC ,0,0,W , H,SRCCOPY);
			pDC->StretchBlt(x1,y1,x2,y2,&MemDC ,0, 0, W, H, SRCCOPY);
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_AreaView diagnostics

#ifdef _DEBUG
void CPro_Free_AreaView::AssertValid() const
{
	CView::AssertValid();
}

void CPro_Free_AreaView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_AreaView message handlers


void CPro_Free_AreaView::ColorMap_Function(CString type)	//	97/12/28
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
...
void CPro_Free_AreaView::OnTopo0(){	ColorMap_Function(_T("Topo_0"));	}
void CPro_Free_AreaView::OnUpdateTopo0(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_0" ? 1 : 0);
}

void CPro_Free_AreaView::OnTopo1(){	ColorMap_Function(_T("Topo_1"));	}
void CPro_Free_AreaView::OnUpdateTopo1(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_1" ? 1 : 0);
}

void CPro_Free_AreaView::OnJet(){	ColorMap_Function(_T("Jet"));	} 
void CPro_Free_AreaView::OnUpdateJet(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Jet" ? 1 : 0);
}

void CPro_Free_AreaView::OnGray(){	ColorMap_Function(_T("Gray"));	}  
void CPro_Free_AreaView::OnUpdateGray(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Gray" ? 1 : 0);
}

void CPro_Free_AreaView::OnPink(){	ColorMap_Function(_T("Pink"));	} 
void CPro_Free_AreaView::OnUpdatePink(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Pink" ? 1 : 0);
}

void CPro_Free_AreaView::OnCopper(){	ColorMap_Function(_T("Copper"));	}  
void CPro_Free_AreaView::OnUpdateCopper(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Copper" ? 1 : 0);
}

void CPro_Free_AreaView::OnGlobe(){	ColorMap_Function(_T("Globe"));	} 
void CPro_Free_AreaView::OnUpdateGlobe(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Globe" ? 1 : 0);
}

void CPro_Free_AreaView::OnSize(UINT nType, int cx, int cy) 
{
...
void CPro_Free_AreaView::OnMouseMove(UINT nFlags, CPoint point) 
{
	///////////////////// StatusBAR/////////////////////////////////////
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
...
lon = ((CSMS4DCApp *)AfxGetApp())->ReverseLon(lon);
	   ...
	   ((CMainFrame*)(AfxGetApp()->m_pMainWnd))->m_wndStatusBar.SetPaneText(5, string);
		...
		double dist=Distance_km(pDoc->Freelat_ST,pDoc->Freelon_ST,lat,lon);
...
void CPro_Free_AreaView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	m_bLPressed=true;
	if (m_LegendFlag)
	{
...
void CPro_Free_AreaView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_bLPressed=false;
	m_bLSelect=false;
...
void CPro_Free_AreaView::DEG2DMS(CString type,double x,int *xD,int *xM,double *xS,char *xU) 
{
...
double CPro_Free_AreaView::Distance_km(double lat1,double lon1,double lat2,double lon2) 
{
...
void CPro_Free_AreaView::OnShowLegend() 
{
...
/*
	...
	CPro_Free_AreaDoc* pDoc=(CPro_Free_AreaDoc*)GetDocument() ;
	m_pLegendDLGModeless->m_title = "E (dBuV/m)";
	m_pLegendDLGModeless->m_MinValue = pDoc->m_MinValue;
	m_pLegendDLGModeless->m_MaxValue = pDoc->m_MaxValue;
	m_pLegendDLGModeless->m_colormaptype=pDoc->colormaptype;

	m_pLegendDLGModeless->m_level = pDoc->m_level;
	m_pLegendDLGModeless->m_bThreshold = pDoc->m_bThreshold;
	m_pLegendDLGModeless->m_Threshold = pDoc->m_Threshold;
...
void CPro_Free_AreaView::OnUpdateShowLegend(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_LegendFlag ? 1 : 0);
}

void CPro_Free_AreaView::OnToolsNormalize() 
{
	CPro_Free_AreaDoc* pDoc = (CPro_Free_AreaDoc*)GetDocument();
...
int CPro_Free_AreaView::Contour(double y,double MinV,double MaxV,int nL) 
{
...
void CPro_Free_AreaView::OnToolsCoveragearea() 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
/*
...
void CPro_Free_AreaView::OnUpdateToolsCoveragearea(CCmdUI* pCmdUI) 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
...
void CPro_Free_AreaView::OnToolsContour() 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
	CContourDLG cdlg;
...
void CPro_Free_AreaView::LegendLOC(CDC* pDC) 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
...
void CPro_Free_AreaView::AreaShow(CDC* pDC) 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
...
BOOL CPro_Free_AreaView::OnPreparePrinting(CPrintInfo* pInfo) 
{
	return DoPreparePrinting(pInfo);
}

void CPro_Free_AreaView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
}
void CPro_Free_AreaView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/) 
{
}
void CPro_Free_AreaView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	if(pInfo)
	{
...
void CPro_Free_AreaView::OnFilePrintPreview() 
{
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarCheck();
	CView::OnFilePrintPreview();
}

void CPro_Free_AreaView::OnEndPrintPreview(CDC* pDC, CPrintInfo* pInfo, POINT point, CPreviewView* pView) 
{
...
void CPro_Free_AreaView::OnToolsContourvalue() 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
	...
	if(vdlg.DoModal()==IDOK)
	{
		...
		Latst = pDoc->Freelat_ST;	Lonst = pDoc->Freelon_ST;
		CString progress_str, progress_char = "|";

		CString str;
		str.Format("%sFreeSpace%d%d%d%d%d.txt",pDoc->FreeName_ST,(int)Lonst,(int)Latst,abs(E),(int)pDoc->FreeRxH,(int)pDoc->Freek);
...
CString CPro_Free_AreaView::str_rep(CString str,int num)
{
	CString str1 = str;
	for(int i=0;i<num;i++)
		str1 = str1+str;
...
void CPro_Free_AreaView::OnDrawVector(CString sFile) 
{
	CVectorDLG Vdlg;
...
void CPro_Free_AreaView::DrawVector(CDC *pDC,CString Vector_filepath,
...
void CPro_Free_AreaView::LatLon2Point(double lat_deg,double lon_deg,CPoint *PointSt) 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
...
lon_deg = ((CSMS4DCApp *)AfxGetApp())->ChangeLon(lon_deg,pDoc->TileX);
	...
	double ix = (double)(pDoc->m_ZoomFactor)*((lon_deg - pDoc->Freelon0)/(pDoc->m_Resolution_x));
	double jy = (double)(pDoc->m_ZoomFactor)*((lat_deg - pDoc->Freelat0)/(pDoc->m_Resolution_x));
...
void CPro_Free_AreaView::OnToolsDrawcontourfromfile() 
{
	CString sFile, sPath;
...
void CPro_Free_AreaView::OnToolsRemovecontourfromdisplay() 
{
	m_POLYnum = 0;
	InvalidateRect(NULL,false);
}
...
/////////////////////////////Google Earth//////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void CPro_Free_AreaView::CaptureDC()
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
	CString str = pDoc->GetTitle();
	CString fileOUTkml = ((CSMS4DCApp *)AfxGetApp())->m_AppPath + _T("Reports\\") + str + _T(".kml");

//	char strFilter[] = {"Google File (*.kml)|*.kml||"};
//	CFileDialog FileDLG(FALSE, ".kml", fileOUTkml, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, strFilter);
...
//		fileOUTkml = FileDLG.GetPathName;
		fileOUTkml = sPath;
...
double Freelon_ST = ((CSMS4DCApp *)AfxGetApp())->ReverseLon(pDoc->Freelon_ST);
...
//		str4.Format(_T("<Placemark><name>%s</name><StyleMap id=\'msn_A\'><Pair><key>normal</key><styleUrl>#sn_A</styleUrl></Pair></StyleMap><Style id=\'sn_A\'><IconStyle><scale>0.9</scale><Icon><href>%s</href></Icon></IconStyle><LabelStyle><scale>0.7</scale></LabelStyle></Style><styleUrl>#msn_A</styleUrl><Point><coordinates>%lf,%lf,0</coordinates></Point></Placemark>"),pDoc->FreeName_ST,gifFile ,pDoc->Freelon_ST,pDoc->Freelat_ST);
		str4.Format(_T("<Placemark><name>%s</name><StyleMap id=\'msn_A\'><Pair><key>normal</key><styleUrl>#sn_A</styleUrl></Pair></StyleMap><Style id=\'sn_A\'><IconStyle><scale>0.9</scale><Icon><href>%s</href></Icon></IconStyle><LabelStyle><scale>0.7</scale></LabelStyle></Style><styleUrl>#msn_A</styleUrl><Point><coordinates>%lf,%lf,0</coordinates></Point></Placemark>"),pDoc->FreeName_ST,gifFile ,      Freelon_ST,pDoc->Freelat_ST);
...
void CPro_Free_AreaView::OnToolsGoogle() 
{
	CaptureDC();
}


void CPro_Free_AreaView::OnFileSaveAs() 
{
	OnFileSave();	
}
void CPro_Free_AreaView::OnFileSave() 
{
	CPro_Free_AreaDoc *pDoc =(CPro_Free_AreaDoc *) GetDocument();
	CString str = pDoc->GetTitle();
...
void CPro_Free_AreaView::OnInitialUpdate() 
{
	CView::OnInitialUpdate();

	CString Title_En  = "Area Calculation (Free Space)";
	CString Title_Num = GetDocument()->GetTitle();		Title_Num.Replace(Title_En,"");
	GetParent()->SetWindowText(_Z(Title_En) + Title_Num); 

	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ChangeMenuLang(IDR_SMS4DCTYPE3);
}

BOOL CPro_Free_AreaView::OnEraseBkgnd(CDC* pDC) //98/01/25
{
	return true;
