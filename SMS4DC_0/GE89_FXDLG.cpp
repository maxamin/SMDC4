// GE89_FXDLG.cpp : implementation file
//
...
CGE89_FXDLG::CGE89_FXDLG(CWnd* pParent /*=NULL*/)
	: CDialog(CGE89_FXDLG::IDD, pParent)
{
	//{{AFX_DATA_INIT(CGE89_FXDLG)
	m_editX = _T("");
	m_editY = _T("");
	m_rowsY = 0;
	//}}AFX_DATA_INIT
	m_title1 = _Z("GE89 : Likely Affected Fixed Stations(BT-->FX)");
	m_title2 = _Z("Likely Affected Fixed Stations : ");
	m_PR_CLS = _T("RPR(dB)" );

}


void CGE89_FXDLG::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CGE89_FXDLG)
...
BEGIN_MESSAGE_MAP(CGE89_FXDLG, CDialog)
	//{{AFX_MSG_MAP(CGE89_FXDLG)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CGE89_FXDLG message handlers

BOOL CGE89_FXDLG::OnInitDialog() 
{
	...
	m_LIST1.SetCol(4);	m_LIST1.SetText(_Z("Frq(MHz)"));
...
//////////////////////////////////////////
	...
	m_LIST2.SetColWidth(4,1000);
	m_LIST2.SetColWidth(5,900);
	m_LIST2.SetColWidth(6,2000);
	...
	m_LIST2.SetCol(4);	m_LIST2.SetText(_Z("Frq(MHz)"));
	m_LIST2.SetCol(5);	m_LIST2.SetText(m_PR_CLS);
...
//	oFileDlg.m_ofn.lpstrTitle = _T("Save HTML file as...");
	if (oFileDlg.DoModal() == IDOK)
	{
		...
		HtmlReport.InsertTableHeader(iCol, _Z("No."), _Z("AssignID"), _Z("Site Name"), _Z("Country"), _Z("Frq(MHz)"));
		for( j = 1; iRow>1; j++)
		{
		...
		HtmlReport1.InsertTableHeader(iCol, _Z("No."), _Z("AssignID"), _Z("Site Name"), _Z("Country"), _Z("Frq(MHz)"), m_PR_CLS, _Z("Location"), _T("Esi(dBuV/m)"), _Z("Elimit(dBuV/m)"));
		for( j = 1; iRow>1; j++)
		{
			ResultSTR.Format("%d",j);
			sCELL[0] = ResultSTR;
