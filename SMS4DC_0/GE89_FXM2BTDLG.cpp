// GE89_FXM2BTDLG.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CGE89_FXM2BTDLG dialog


CGE89_FXM2BTDLG::CGE89_FXM2BTDLG(CWnd* pParent /*=NULL*/)
	: CDialog(CGE89_FXM2BTDLG::IDD, pParent)
{
	//{{AFX_DATA_INIT(CGE89_FXM2BTDLG)
	m_editX = _T("");
	m_editY = _T("");
	m_rowsY = 0;
	//}}AFX_DATA_INIT
	m_title1 = _Z("GE89 : Interference from wanted station to selected stations");
	m_title2 = _Z("Interference to : ");
}


void CGE89_FXM2BTDLG::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CGE89_FXM2BTDLG)
...
BEGIN_MESSAGE_MAP(CGE89_FXM2BTDLG, CDialog)
	//{{AFX_MSG_MAP(CGE89_FXM2BTDLG)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CGE89_FXM2BTDLG message handlers

BOOL CGE89_FXM2BTDLG::OnInitDialog() 
{
	...
	m_LIST1.SetCol(4);	m_LIST1.SetText(_Z("Assign Frq(MHz)"));
...
//////////////////////////////////////////
	...
	m_LIST2.SetColWidth(5,900);
	m_LIST2.SetColWidth(6,1200);
	m_LIST2.SetColWidth(7,1400);
	...
	m_LIST2.SetCol(4);	m_LIST2.SetText(_Z("Assign Frq(MHz)"));
	m_LIST2.SetCol(5);	m_LIST2.SetText(_T("RPR(dB)"));
	m_LIST2.SetCol(6);	m_LIST2.SetText(_T("Em(dBuV/m)"));
	m_LIST2.SetCol(7);	m_LIST2.SetText(_Z("Elimit(dBuV/m)"));
	...
	for(j=0;j<m_rowsY-1;j++)
	{
		if(   atof(m_LIST2.GetTextMatrix(j+1,6)) > atof(m_LIST2.GetTextMatrix(j+1,7))  )
		{
			m_LIST2.SetCol(6);
			m_LIST2.SetRow(j+1);
...
void CGE89_FXM2BTDLG::OnOK() 
{
	...
	if (oFileDlg.DoModal() == IDOK)
	{
		...
		HtmlReport.InsertTableHeader(iCol, _Z("No."), _Z("AssignID"), _Z("Site Name"), _Z("Country"), _Z("Assign Frq(MHz)"));
		for( j = 1; iRow>1; j++)
		{
		...
		HtmlReport1.InsertTableHeader(iCol, _Z("No."), _Z("AssignID"), _Z("Site Name"), _Z("Country"), _Z("Assign Frq(MHz)"), _T("RPR(dB)"), _T("Em(dBuV/m)"), _Z("Elimit(dBuV/m)"));
		for( j = 1; iRow>1; j++)
		{
			ResultSTR.Format("%d",j);
			sCELL[0] = ResultSTR;
