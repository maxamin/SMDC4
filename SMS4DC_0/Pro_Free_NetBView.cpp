// Pro_Free_NetBView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_Free_NetBView, CView)

CPro_Free_NetBView::CPro_Free_NetBView()
{
	old_cx=old_cy=600;
	new_cx=new_cy=600;
	m_LegendFlag=false;
...
BEGIN_MESSAGE_MAP(CPro_Free_NetBView, CView)
	//{{AFX_MSG_MAP(CPro_Free_NetBView)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_NetBView drawing

#include "memdc.h"
void CPro_Free_NetBView::OnDraw(CDC* pDC)	//98/01/25
{
	CMemDC mDC(pDC);
	OnDraw4(mDC);
}
void CPro_Free_NetBView::OnDraw4(CDC* pDC)	//98/01/25
{
	CPro_Free_NetBDoc *pDoc =(CPro_Free_NetBDoc *) GetDocument();
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_NetBView diagnostics

#ifdef _DEBUG
void CPro_Free_NetBView::AssertValid() const
{
	CView::AssertValid();
}

void CPro_Free_NetBView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_NetBView message handlers

void CPro_Free_NetBView::OnSize(UINT nType, int cx, int cy) 
{
...
void CPro_Free_NetBView::OnShowLegend() 
{
...
void CPro_Free_NetBView::OnUpdateShowLegend(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_LegendFlag ? 1 : 0);
}

void CPro_Free_NetBView::OnMouseMove(UINT nFlags, CPoint point) 
{
	///////////////////// StatusBAR/////////////////////////////////////
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
//Monitoring 98/12/17
CString Cov1[3];
if(m_modeFlagFreeNetB == 2)
{
	Cov1[0] = _Z("Listening");
	Cov1[1] = _Z("Measurement");
	Cov1[2] = _Z("Direction-Finding");
}
else if(m_modeFlagFreeNetB == 3)
{
	Cov1[0] = "TDoA";
}

if     (m_modeFlagFreeNetB == 0)	string.Format(_Z("Server :")+" %s",( (E==0) ? _Z("No Station") : pDoc->FreeName_ST[E-1]) ); 		//Best Server
else if(m_modeFlagFreeNetB == 1)	string.Format(_Z("Server :")+" %s",( (E==0) ? _Z("No Station") : pDoc->FreeName_ST[E-1]) );		//Monitoring Rx Coverage(BS)
else if(m_modeFlagFreeNetB == 2)	string.Format(_Z("Coverage")+" : %s",( (E==0) ? _Z("No Coverage") : Cov1[E-1]) );					//Monitoring coverage zones
else if(m_modeFlagFreeNetB == 3)	string.Format(_Z("Coverage")+" : %s", ( (E==0) ? _Z("No TDoA") : Cov1[E-1]) );						//Monitoring  TDoA

//		string.Format(_Z("Server :")+" %s", ( (E==0) ? _Z("No Station") : pDoc->FreeName_ST[E-1]) );
...
void CPro_Free_NetBView::DEG2DMS(CString type,double x,int *xD,int *xM,double *xS,char *xU) 
{
	if (x < 0)
	{
		x=-x;
...
void CPro_Free_NetBView::OnToolsThreshold() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnTopo0() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnUpdateTopo0(CCmdUI* pCmdUI) 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_0" ? 1 : 0);
}

void CPro_Free_NetBView::OnTopo1() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnUpdateTopo1(CCmdUI* pCmdUI) 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_1" ? 1 : 0);
}

void CPro_Free_NetBView::OnJet() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnUpdateJet(CCmdUI* pCmdUI) 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Jet" ? 1 : 0);
}

void CPro_Free_NetBView::OnGray() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnUpdateGray(CCmdUI* pCmdUI) 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Gray" ? 1 : 0);
}

void CPro_Free_NetBView::OnPink() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnUpdatePink(CCmdUI* pCmdUI) 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Pink" ? 1 : 0);
}

void CPro_Free_NetBView::OnCopper() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnUpdateCopper(CCmdUI* pCmdUI) 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Copper" ? 1 : 0);
}

void CPro_Free_NetBView::OnGlobe() 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::OnUpdateGlobe(CCmdUI* pCmdUI) 
{
	CPro_Free_NetBDoc* pDoc = (CPro_Free_NetBDoc*)GetDocument();
...
void CPro_Free_NetBView::LegendLOC(CDC* pDC) 
{
	CPro_Free_NetBDoc *pDoc =(CPro_Free_NetBDoc *) GetDocument();

	if     (m_modeFlagFreeNetB == 0)	LegendLOC0(pDC); 		//Best Server
	else if(m_modeFlagFreeNetB == 1)	LegendLOC0(pDC);		//Monitoring Rx Coverage(BS)
	else if(m_modeFlagFreeNetB == 2)	LegendLOC1(pDC);		//Monitoring coverage zones
	else if(m_modeFlagFreeNetB == 3)	LegendLOC2(pDC);		//Monitoring  TDoA
}

void CPro_Free_NetBView::LegendLOC0(CDC* pDC) 
{
	CPro_Free_NetBDoc *pDoc =(CPro_Free_NetBDoc *) GetDocument();
...
//		RGBt[i]=m_tile.RGBt[ tt*(i) ];	//98/01/19
	...
	for ( i=0;i<Nrow;i++)
	{
		label.Format("%s",pDoc->FreeName_ST[Nrow-1-i]);
...
void CPro_Free_NetBView::LegendLOC1(CDC* pDC) 
{
	CPro_Free_NetBDoc *pDoc =(CPro_Free_NetBDoc *) GetDocument();
	int x1,y1,x2,y2,w=10,pscale=12,Nrow=pDoc->Nrow;
...
//Monitoring TDOA
void CPro_Free_NetBView::LegendLOC2(CDC* pDC) 
{
	CPro_Free_NetBDoc *pDoc =(CPro_Free_NetBDoc *) GetDocument();
	int x1,y1,x2,y2,w=10,pscale=12,Nrow=pDoc->Nrow;
...
void CPro_Free_NetBView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	m_bLPressed=true;
	if (m_LegendFlag)
	{
...
void CPro_Free_NetBView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_bLPressed=false;
	m_bLSelect=false;
...
BOOL CPro_Free_NetBView::OnPreparePrinting(CPrintInfo* pInfo) 
{
	return DoPreparePrinting(pInfo);
}

void CPro_Free_NetBView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
}
void CPro_Free_NetBView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/) 
{
}
void CPro_Free_NetBView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	if(pInfo)
	{
...
void CPro_Free_NetBView::OnFilePrintPreview() 
{
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarCheck();
	CView::OnFilePrintPreview();
}

void CPro_Free_NetBView::OnEndPrintPreview(CDC* pDC, CPrintInfo* pInfo, POINT point, CPreviewView* pView) 
{
...
/////////////////////////////Google Earth//////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void CPro_Free_NetBView::CaptureDC()
{
	CPro_Free_NetBDoc *pDoc =(CPro_Free_NetBDoc *) GetDocument();
...
void CPro_Free_NetBView::OnToolsGoogle() 
{
	CaptureDC();
}

void CPro_Free_NetBView::OnFileSaveAs() 
{
	OnFileSave();	
}
void CPro_Free_NetBView::OnFileSave() 
{
	CPro_Free_NetBDoc *pDoc =(CPro_Free_NetBDoc *) GetDocument();
	CString str = pDoc->GetTitle();
...
void CPro_Free_NetBView::OnInitialUpdate() 
{
	CView::OnInitialUpdate();

	CString Title_En  = "Best Server";		//98/12/17
//	CString Title_En  = "Best Server (Free Space)";
	CString Title_Num = GetDocument()->GetTitle();		Title_Num.Replace(Title_En,"");

//Monitoring	98/12/17
	m_modeFlagFreeNetB = ((CSMS4DCApp *)AfxGetApp())->m_modeFlagFreeNetB;

	if     (m_modeFlagFreeNetB == 1)	Title_En  = "Monitoring Rx Coverage(BS)";	//Monitoring Rx Coverage(BS)
	else if(m_modeFlagFreeNetB == 2)	Title_En  = "Coverage Zones";				//Monitoring coverage zones
	else if(m_modeFlagFreeNetB == 3)	Title_En  = "Location Zone TDoA";			//Monitoring  TDoA


	GetParent()->SetWindowText(_Z(Title_En) + Title_Num); 
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ChangeMenuLang(IDR_SMS4DCTYPE12);
}

BOOL CPro_Free_NetBView::OnEraseBkgnd(CDC* pDC) 	//98/01/25
{
	return true;
