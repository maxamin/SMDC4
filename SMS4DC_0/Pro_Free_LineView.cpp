// Pro_Free_LineView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_Free_LineView, CView)

CPro_Free_LineView::CPro_Free_LineView()
{
	m_AxesT=0;
	m_AxesH=0;
	m_AxesL=0;
...
BEGIN_MESSAGE_MAP(CPro_Free_LineView, CView)
	//{{AFX_MSG_MAP(CPro_Free_LineView)
...
void CPro_Free_LineView::OnDraw(CDC* pDC)
{
	CPro_Free_LineDoc* pDoc = (CPro_Free_LineDoc*)GetDocument();
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_LineView diagnostics

#ifdef _DEBUG
void CPro_Free_LineView::AssertValid() const
{
	CView::AssertValid();
}

void CPro_Free_LineView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_LineView message handlers

void CPro_Free_LineView::Plot(CDC* pDC,double *xi, double *yi,int N,
...
void CPro_Free_LineView::Xtick(CDC* pDC,double xiMIN,double xiMAX) 
{
	CPro_Free_LineDoc* pDoc = (CPro_Free_LineDoc*)GetDocument();
	CString label;
...
void CPro_Free_LineView::TextDraw(CDC* pDC,int x, int y,CString str,COLORREF FontColor,
							int Escapement,
...
void CPro_Free_LineView::LineDraw(CDC* pDC,int x1, int y1,int x2, int y2,
...
double CPro_Free_LineView::Distance_km(double lat1,double lon1,double lat2,double lon2) 
{
...
void CPro_Free_LineView::OnRefresh() 
{
	Invalidate();	
}

void CPro_Free_LineView::YtickL(CDC* pDC,double yiMIN,double yiMAX) 
{
	CString label;
...
void CPro_Free_LineView::YtickR(CDC* pDC,double yiMIN,double yiMAX) 
{
	CString label;
	double r,step_m,yround=0,yiMAXr;
	int y=0,yNum=20;
...
double CPro_Free_LineView::MinValue(double *xi,int N) 
{
...
double CPro_Free_LineView::MaxValue(double *xi,int N) 
{
...
void CPro_Free_LineView::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CPro_Free_LineView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_bLPressed=false;
	CView::OnLButtonUp(nFlags, point);
}

void CPro_Free_LineView::OnMouseMove(UINT nFlags, CPoint point) 
{
	CPro_Free_LineDoc* pDoc = (CPro_Free_LineDoc*)GetDocument();
...
void CPro_Free_LineView::DEG2DMS(CString type,double x,int *xD,int *xM,double *xS,char *xU) 
{
...
int CPro_Free_LineView::Round(double x) 
{
	int x1 = (int) x;
	int x2 = x1+1;
...
double CPro_Free_LineView::Interp(double *hi , int Np , double x) 
{
	int x1,x2;
	double y;
...
double CPro_Free_LineView::InterpXY(double *D0,double *E0,double d,int num)
{
...
void CPro_Free_LineView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	if(pInfo)
		m_rcPrintRect=pInfo->m_rectDraw;
	CView::OnPrint(pDC, pInfo);
}
void CPro_Free_LineView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
}
void CPro_Free_LineView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/) 
{
}
BOOL CPro_Free_LineView::OnPreparePrinting(CPrintInfo* pInfo) 
{
	return DoPreparePrinting(pInfo);
}

void CPro_Free_LineView::OnFilePrintPreview() 
{
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarCheck();
	CView::OnFilePrintPreview();
}

void CPro_Free_LineView::OnEndPrintPreview(CDC* pDC, CPrintInfo* pInfo, POINT point, CPreviewView* pView) 
{
...
void CPro_Free_LineView::OnInitialUpdate() 
{
	CView::OnInitialUpdate();

	CString Title_En  = "Line Calculation (Free Space)";
	CString Title_Num = GetDocument()->GetTitle();		Title_Num.Replace(Title_En,"");
	GetParent()->SetWindowText(_Z(Title_En) + Title_Num); 

	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ChangeMenuLang(IDR_SMS4DCTYPE2);
}
