// Pro_Free_NetFView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_Free_NetFView, CView)

CPro_Free_NetFView::CPro_Free_NetFView()
{
	old_cx=old_cy=600;
	new_cx=new_cy=600;
	m_LegendFlag=false;
...
BEGIN_MESSAGE_MAP(CPro_Free_NetFView, CView)
	//{{AFX_MSG_MAP(CPro_Free_NetFView)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_NetFView drawing

#include "memdc.h"
void CPro_Free_NetFView::OnDraw(CDC* pDC)	//98/01/25
{
	CMemDC mDC(pDC);
	OnDraw4(mDC);
}
void CPro_Free_NetFView::OnDraw4(CDC* pDC)	//98/01/25
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_NetFView diagnostics

#ifdef _DEBUG
void CPro_Free_NetFView::AssertValid() const
{
	CView::AssertValid();
}

void CPro_Free_NetFView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_Free_NetFView message handlers

void CPro_Free_NetFView::OnSize(UINT nType, int cx, int cy) 
{
...
void CPro_Free_NetFView::OnToolsNormalize() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnShowLegend() 
{
...
void CPro_Free_NetFView::OnUpdateShowLegend(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_LegendFlag ? 1 : 0);
}

void CPro_Free_NetFView::OnMouseMove(UINT nFlags, CPoint point) 
{
	///////////////////// StatusBAR/////////////////////////////////////
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
//		if(m_modeFlagFreeNetF == 2)	string.Format("Uncertainty (km) : %0.3f",E );	//Monitoring  AoA
		if(m_modeFlagFreeNetF == 2)	string.Format(_Z("Uncertainty (km)") + " : %0.3f",E );	//Monitoring  AoA
...
void CPro_Free_NetFView::DEG2DMS(CString type,double x,int *xD,int *xM,double *xS,char *xU) 
{
	if (x < 0)
	{
		x=-x;
...
void CPro_Free_NetFView::OnTopo0() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnUpdateTopo0(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_0" ? 1 : 0);
}

void CPro_Free_NetFView::OnTopo1() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnUpdateTopo1(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_1" ? 1 : 0);
}

void CPro_Free_NetFView::OnJet() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnUpdateJet(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Jet" ? 1 : 0);
}

void CPro_Free_NetFView::OnGray() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnUpdateGray(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Gray" ? 1 : 0);
}

void CPro_Free_NetFView::OnPink() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnUpdatePink(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Pink" ? 1 : 0);
}

void CPro_Free_NetFView::OnCopper() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnUpdateCopper(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Copper" ? 1 : 0);
}

void CPro_Free_NetFView::OnGlobe() 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
...
void CPro_Free_NetFView::OnUpdateGlobe(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc* pDoc = (CPro_Free_NetFDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Globe" ? 1 : 0);
}

void CPro_Free_NetFView::OnToolsContour() 
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
	CContourDLG cdlg;
...
void CPro_Free_NetFView::OnToolsCoveragearea() 
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
/*
...
void CPro_Free_NetFView::OnUpdateToolsCoveragearea(CCmdUI* pCmdUI) 
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
	pCmdUI->SetCheck(pDoc->m_bThreshold);
}

int CPro_Free_NetFView::Contour(double y,double MinV,double MaxV,int nL) 
{
...
void CPro_Free_NetFView::LegendLOC(CDC* pDC) 
{
	if     (m_modeFlagFreeNetF == 0)	LegendLOC0( pDC);	//Maximum Field Strength(MFS)
	else if(m_modeFlagFreeNetF == 1)	LegendLOC0( pDC);	//Monitoring Rx Coverage
	else if(m_modeFlagFreeNetF == 2)	LegendLOC1( pDC);	//Monitoring  AoA
}

void CPro_Free_NetFView::LegendLOC0(CDC* pDC) 
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
...
void CPro_Free_NetFView::LegendLOC1(CDC* pDC) 
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
	int x1,y1,x2,y2,w=10,h=1,pscale=12;
...
//			RGBt[i]=m_tile.RGBt[ 0+(1-i)*159 ];			//98/01/19
	...
	else
	{
		...
		{
			RGBt[i]=m_tile.RGBt[ tt*(i/tt) ];
//			RGBt[i]=m_tile.RGBt[ 255-tt*(i/tt) ];			//990431
			CPoint pt(x1+5*pscale, y1+(30+255-i)*pscale);
			CRect rect3(pt, sz);
			CBrush brush3(RGBt[i]);
			pDC->SelectObject(&brush3);
...
//	pDC->TextOut(x1+5*pscale,y1+5*pscale,"delta D (km)");
//	pDC->TextOut(x1+5*pscale,y1+5*pscale,"E (dBuV/m)");
	pDC->SetTextAlign(TA_RIGHT);
...
void CPro_Free_NetFView::AreaShow(CDC* pDC) 
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
...
void CPro_Free_NetFView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	m_bLPressed=true;
	if (m_LegendFlag)
	{
...
void CPro_Free_NetFView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_bLPressed=false;
	m_bLSelect=false;
...
BOOL CPro_Free_NetFView::OnPreparePrinting(CPrintInfo* pInfo) 
{
	return DoPreparePrinting(pInfo);
}

void CPro_Free_NetFView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
}
void CPro_Free_NetFView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/) 
{
}
void CPro_Free_NetFView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	if(pInfo)
	{
...
void CPro_Free_NetFView::OnFilePrintPreview() 
{
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarCheck();
	CView::OnFilePrintPreview();
}

void CPro_Free_NetFView::OnEndPrintPreview(CDC* pDC, CPrintInfo* pInfo, POINT point, CPreviewView* pView) 
{
...
/////////////////////////////Google Earth//////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void CPro_Free_NetFView::CaptureDC()
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
...
void CPro_Free_NetFView::OnToolsGoogle() 
{
	CaptureDC();
}

void CPro_Free_NetFView::OnFileSaveAs() 
{
	OnFileSave();	
}
void CPro_Free_NetFView::OnFileSave() 
{
	CPro_Free_NetFDoc *pDoc =(CPro_Free_NetFDoc *) GetDocument();
	CString str = pDoc->GetTitle();
...
void CPro_Free_NetFView::OnInitialUpdate() 
{
	CView::OnInitialUpdate();

	CString Title_En  = "Maximum Field Strength";		//98/12/17
//	CString Title_En  = "Maximum Field Strength (Free Space)";
	CString Title_Num = GetDocument()->GetTitle();		Title_Num.Replace(Title_En,"");

//Monitoring		98/12/17
	m_modeFlagFreeNetF = ((CSMS4DCApp *)AfxGetApp())->m_modeFlagFreeNetF;

	if     (m_modeFlagFreeNetF == 1)	Title_En  = "Monitoring Rx Coverage(MFS)";	//Monitoring Rx Coverage
	else if(m_modeFlagFreeNetF == 2)	Title_En  = "Location Zone AoA";			//Monitoring  AoA

	GetParent()->SetWindowText(_Z(Title_En) + Title_Num); 
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ChangeMenuLang(IDR_SMS4DCTYPE11);
}

BOOL CPro_Free_NetFView::OnEraseBkgnd(CDC* pDC) 	//98/01/25
{
	return true;
