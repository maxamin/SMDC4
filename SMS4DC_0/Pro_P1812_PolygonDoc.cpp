// Pro_P1812_PolygonDoc.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1812_PolygonDoc

IMPLEMENT_DYNCREATE(CPro_P1812_PolygonDoc, CDocument)

CPro_P1812_PolygonDoc::CPro_P1812_PolygonDoc()
{
	m_ReadyDoc=0;
	m_ZoomFactor=16;
...
BOOL CPro_P1812_PolygonDoc::OnNewDocument()
{
...
BEGIN_MESSAGE_MAP(CPro_P1812_PolygonDoc, CDocument)
	//{{AFX_MSG_MAP(CPro_P1812_PolygonDoc)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CPro_P1812_PolygonDoc diagnostics

#ifdef _DEBUG
void CPro_P1812_PolygonDoc::AssertValid() const
{
	CDocument::AssertValid();
}

void CPro_P1812_PolygonDoc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1812_PolygonDoc serialization

void CPro_P1812_PolygonDoc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1812_PolygonDoc commands
...
void CPro_P1812_PolygonDoc::GetData()
{
	...
	double elevp,tp,fp,az,f,t, rearth=6371.0, dist,Ghv=1.0;;
...
//////////////////////////////////////////////////
	...
	int pol = 1;
	if     (LinePolarizationTx=="H") pol = 0;
	else if(LinePolarizationTx=="V") pol = 1;

	double Lb, f_GHz = LinefMHz_ST/1000.0;
	...
	DELTA_N = dat_filesDATAout(Linelat_ST,Linelon_ST, dataDN50);
	Linek = 157.0 / (157.0 - DELTA_N);	// Equation (6)
	double re = Linek * rearth * 1000.0;

	double LinePtGt_STp = LinePtGt_ST/1650.0;
	...
	for ( i=0;i<Np;i++)
	{
		...
		{
			reckon(Linelat_ST, Linelon_ST, dist/2.0, az, &LatM, &LongM) ;
			...
			while(ds<dist)
			{
				reckon(Linelat_ST, Linelon_ST, ds, az, &latp, &lonp);
			...
			mZ[Np] = LandSeaCoastal(m_loni[i], m_lati[i]);
			Np++;
			mZ[Np] = 0;

			Lb = BasicTxLossP1812V3(mD, mH, mZ, Np, LatM, DELTA_N, N0,
									Linetime, Linelocation, f_GHz, Ht, Hr, pol, 
									LineSrv, LineIODoor, LineClutter);
	...
	//	elevp = atan(((re+Hr    )*cos(dist1000_re)-(re+Ht+Hg))/((re+Hr    )*sin(dist1000_re)));  //without Terrain
		...
		double Eb = 199.36 + 20.0*log10(f_GHz) - Lb + 10.0*log10(LinePtGt_STp*Ghv);
...
double CPro_P1812_PolygonDoc::Distance_km(double lat1,double lon1,double lat2,double lon2) 
{
...
void CPro_P1812_PolygonDoc::ReadAntennaData(CString antfile,int *f0,double *rH,double *rV) 
{
	FILE *fid;
...
double CPro_P1812_PolygonDoc::Azimuth_Deg(double lat1,double lon1,double lat2,double lon2) 
{
	float	lat10 = (float)(lat1*pi/180.0),
			lon10 = (float)(lon1*pi/180.0),
			lat20 = (float)(lat2*pi/180.0),
...
double CPro_P1812_PolygonDoc::Interp2(int *D0,double *E0,double d,int num) 
{
...
void CPro_P1812_PolygonDoc::reckon(double lat_deg,double lon_deg,double rng_km,double az_deg,double *latC_deg,double *lonC_deg) 
{
...
void CPro_P1812_PolygonDoc::LatLon2Point(double lat_deg,double lon_deg,CPoint *PointSt) 
{
...
double CPro_P1812_PolygonDoc::Point2Hg(CPoint point1) 
{
	int i1=point1.x   ,   j1=(Height-1)-point1.y;
	_int16 Hg=bufArea[i1+Width*j1];
	return (double)Hg;
}
double CPro_P1812_PolygonDoc::LatLon2Hg0(double lat_deg,double lon_deg) 
{
	CPoint pointi;
...
double CPro_P1812_PolygonDoc::LatLon2Hg(double lat_deg,double lon_deg)
{
	lon_deg = ((CSMS4DCApp *)AfxGetApp())->ChangeLon(lon_deg,TileX);
...
double CPro_P1812_PolygonDoc::RoundBUF_Hg(double xps,double yps) 
{
...
double CPro_P1812_PolygonDoc::Points2HgAvr(CPoint point1 , CPoint point2) 
{
	double HgAvr = 0.0;
	if (point1 != point2)
	{
...
double CPro_P1812_PolygonDoc::TCAcorr1546(double Tdeg,double f_MHz)
{
...
int CPro_P1812_PolygonDoc::Round(double x) 
{
	int x1 = (int) x;
	int x2 = x1+1;
...
int CPro_P1812_PolygonDoc::WarmColdSea(double RLON_deg,double RLAT_deg) 
{
...
int CPro_P1812_PolygonDoc::LandWarmColdSea(double RLON_deg,double RLAT_deg) 
{
...
double CPro_P1812_PolygonDoc::D_Sea_kmIDWM(double lat1_deg,double lon1_deg,double lat2_deg,double lon2_deg,
									   double *D_SeaW_km)  
{
...
void CPro_P1812_PolygonDoc::WarmColdCross(double RLON_deg1,double RLAT_deg1,double RLON_deg2,double RLAT_deg2,
...
double CPro_P1812_PolygonDoc::LandWarmColdSea_D_Sea_kmIDWM(double lat1_deg,double lon1_deg,double lat2_deg,double lon2_deg,
...
void CPro_P1812_PolygonDoc::LoadIDWMmap() 
{
...
int CPro_P1812_PolygonDoc::LandSeaCoastal(double RLON_deg,double RLAT_deg) 
{
...
int CPro_P1812_PolygonDoc::dat_filesDATA(CString dat_fname, float data[121][241])
{
	FILE *fp;
...
double CPro_P1812_PolygonDoc::dat_filesDATAout(double LatM,double LongM, float data[121][241])
{
