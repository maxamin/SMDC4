// Pro_P370_LineDoc.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_P370_LineDoc, CDocument)

CPro_P370_LineDoc::CPro_P370_LineDoc()
{
	m_ZoomFactor=16;
}

BOOL CPro_P370_LineDoc::OnNewDocument()
{
...
BEGIN_MESSAGE_MAP(CPro_P370_LineDoc, CDocument)
	//{{AFX_MSG_MAP(CPro_P370_LineDoc)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CPro_P370_LineDoc diagnostics

#ifdef _DEBUG
void CPro_P370_LineDoc::AssertValid() const
{
	CDocument::AssertValid();
}

void CPro_P370_LineDoc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P370_LineDoc serialization

void CPro_P370_LineDoc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P370_LineDoc commands


void CPro_P370_LineDoc::GetData() 
{
...
dist = max(dist , 0.0005);
		...
		if (h1<0.0)
		{
			...
			while( ds<= mds )  //ds=0->15
			{
				...
				hi=Point2Hg(pointi);
				tetai=max(tetai   ,(((hi-Hg-LineHagl_ST)/(ds*1000))-(ds/(2.0*Linek*r))));
			}
		...
		if (LineClangle==1)
		{
			...
			while( dz<= mdz )  //dz=0->16
			{
				...
				LatLon2Point(lat015km , lon015km, &pointi) ;
				hii=Point2Hg(pointi);
				tetamax16=max(tetamax16   ,(((hii-10.0-m_hi[i])/(dz*1000))-(dz/(2.0*Linek*r))));
			}
...
double CPro_P370_LineDoc::Distance_km(double lat1,double lon1,double lat2,double lon2) 
{
...
void CPro_P370_LineDoc::ReadAntennaData(CString antfile,int *f0,double *rH,double *rV) 
{
	FILE *fid;
...
double CPro_P370_LineDoc::Azimuth_Deg(double lat1,double lon1,double lat2,double lon2) 
{
	double pi=4.0*atan(1.0);
	float	lat10 = (float)(lat1*pi/180.0),
			lon10 = (float)(lon1*pi/180.0),
...
int CPro_P370_LineDoc::Round(double x) 
{
	int x1 = (int) x;
	int x2 = x1+1;
...
double CPro_P370_LineDoc::Interp2(int *D0,double *E0,double d,int num) 
{
...
void CPro_P370_LineDoc::reckon(double lat_deg,double lon_deg,double rng_km,double az_deg,double *latC_deg,double *lonC_deg) 
{
	double pi=4.0*atan(1.0);
...
void CPro_P370_LineDoc::LatLon2Point(double lat_deg,double lon_deg,CPoint *PointSt) 
{
...
double CPro_P370_LineDoc::Points2HgAvr(CPoint point1 , CPoint point2) 
{
	double HgAvr = 0.0;
	if (point1 != point2)
	{
...
double CPro_P370_LineDoc::Point2Hg(CPoint point1) 
{
...
int CPro_P370_LineDoc::WarmColdSea(double RLON_deg,double RLAT_deg) 
{
...
int CPro_P370_LineDoc::LandWarmColdSea(double RLON_deg,double RLAT_deg) 
{
...
double CPro_P370_LineDoc::D_Sea_kmIDWM(double lat1_deg,double lon1_deg,double lat2_deg,double lon2_deg,
									   double *D_SeaW_km)  
{
	double pi = 4.0*atan(1.0);
...
void CPro_P370_LineDoc::WarmColdCross(double RLON_deg1,double RLAT_deg1,double RLON_deg2,double RLAT_deg2,
