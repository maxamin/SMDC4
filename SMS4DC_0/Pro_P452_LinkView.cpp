// Pro_P452_LinkView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_P452_LinkView, CFormView)

CPro_P452_LinkView::CPro_P452_LinkView()
	: CFormView(CPro_P452_LinkView::IDD)
{
	//{{AFX_DATA_INIT(CPro_P452_LinkView)
	m_Frequency = 0.0;
	m_Gr = 0.0;
	m_Gt = 0.0;
	m_Hr = 0.0;
...
void CPro_P452_LinkView::DoDataExchange(CDataExchange* pDX)
{
	CFormView::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CPro_P452_LinkView)
	...
	DDX_Text(pDX, IDC_EDIT_total, m_totalloss1);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CPro_P452_LinkView, CFormView)
	//{{AFX_MSG_MAP(CPro_P452_LinkView)
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P452_LinkView diagnostics

#ifdef _DEBUG
void CPro_P452_LinkView::AssertValid() const
{
	CFormView::AssertValid();
}

void CPro_P452_LinkView::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P452_LinkView message handlers


void CPro_P452_LinkView::Plot(CDC* pDC,double *xi, double *yi,int N,
...
void CPro_P452_LinkView::DrawAntSymbol(CDC* pDC,double xi, double yi,
						double xiMIN,double xiMAX,double yiMIN,double yiMAX,
...
void CPro_P452_LinkView::TextDraw(CDC* pDC,int x, int y,CString str,COLORREF FontColor,
							int Escapement,
...
void CPro_P452_LinkView::LineDraw(CDC* pDC,int x1, int y1,int x2, int y2,
...
void CPro_P452_LinkView::DEG2DMS(CString type,double x,int *xD,int *xM,double *xS,char *xU) 
{
	if (x < 0)
	{
		x=-x;
...
void CPro_P452_LinkView::YtickL(CDC* pDC,double yiMIN,double yiMAX) 
{
...
void CPro_P452_LinkView::Xtick(CDC* pDC,double xiMIN,double xiMAX) 
{
	CPro_P452_LinkDoc* pDoc = (CPro_P452_LinkDoc *)GetDocument();
	CString label;
...
double CPro_P452_LinkView::MinValue(double *xi,int N) 
{
...
double CPro_P452_LinkView::MaxValue(double *xi,int N) 
{
...
double CPro_P452_LinkView::InterpXY(double *D0,double *E0,double d,int num)
{
...
void CPro_P452_LinkView::OnDraw(CDC* pDC) 
{
	CPro_P452_LinkDoc* pDoc = (CPro_P452_LinkDoc *)GetDocument();
...
void CPro_P452_LinkView::InitForm(CPro_P452_LinkDoc* pDoc) 
{
	UpdateData();
	m_Hr = pDoc->LinkRxH;
...
/*
	...
	if ((pDoc->m_path_type==PN452_LOS_CLEAR)||(pDoc->m_path_type==PN452_ERROR))		m_TotalLoss = atof(m_Loss1);
	else if (pDoc->m_path_type==PN452_LOS_SUB_P)									m_TotalLoss = atof(m_Loss3);
	else if (pDoc->m_path_type==PN452_TRANS_HOZ)									m_TotalLoss = atof(m_Loss4);
*/
	m_TotalLoss = atof(pDoc->m_Loss5);

	m_NFD.Format("%0.3f",pDoc->m_NFD_Loss);

	m_totalloss1.Format("%g",m_TotalLoss+pDoc->m_NFD_Loss);
	m_TotalLoss = atof(m_totalloss1);

	UpdateData(false);
}


void CPro_P452_LinkView::TxRxObj(CDC* pDC) 
{
...
void CPro_P452_LinkView::DrawRect(CDC* pDC,int x1,int y1,int x2,int y2,COLORREF flColor) 
{
	CBrush brush(flColor);
...
void CPro_P452_LinkView::DrawRoundRect(CDC* pDC,int x1,int y1,int x2,int y2,COLORREF flColor) 
{
	CBrush brush(flColor);
	CBrush *oldbrush = pDC->SelectObject(&brush);
...
void CPro_P452_LinkView::OnBUTTONLink() 
{
	UpdateData();
...
void CPro_P452_LinkView::OnRadioLR() 
{
	UpdateData();
...
void CPro_P452_LinkView::OnRadioRL() 
{
	OnRadioLR();
}

void CPro_P452_LinkView::OnInitialUpdate() 
{
	CFormView::OnInitialUpdate();

//	initDP_LP();

	CString Title_En  = "Link Calculation (P452)";
	...
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_timePC);
	xx->SetFont(&font);
	...
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_Loss3);
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_Loss4);
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_FsLoss);
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_Frequncy);
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_FrequncyR);
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_k);
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_NFD);
	xx->SetFont(&font);
	xx = (CEdit*)GetDlgItem(IDC_EDIT_total);
	xx->SetFont(&font);
	...
	UpdateData(false);	
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ChangeMenuLang(IDR_SMS4DCTYPE38);
	((CSMS4DCApp *)AfxGetApp())->TranslateDialog((CDialog *)this);
}

void CPro_P452_LinkView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	if(pInfo)
...
//	int nHeight = 95;
	...
	str.Format(_Z("Time percent")+"(%%) : %g",m_timePC);
	TextDraw(pDC,x+2*w,y+h/2+2*h,str,RGB(0,0,127),0,nHeight,0,FW_BOLD);
	str.Format(_Z("k_factor : ")+"%g",m_kfactor);
...
void CPro_P452_LinkView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
}
void CPro_P452_LinkView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/) 
{
}
BOOL CPro_P452_LinkView::OnPreparePrinting(CPrintInfo* pInfo) 
{
...
void CPro_P452_LinkView::OnFilePrintPreview() 
{
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarCheck();
	CFormView::OnFilePrintPreview();
}
void CPro_P452_LinkView::OnEndPrintPreview(CDC* pDC, CPrintInfo* pInfo, POINT point, CPreviewView* pView) 
{
	CFormView::OnEndPrintPreview(pDC, pInfo, point, pView);
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarSet();
}

HBRUSH CPro_P452_LinkView::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	if (  pWnd->GetDlgCtrlID() == IDC_EDIT_Distance		||
		  pWnd->GetDlgCtrlID() == IDC_EDIT_timePC		||
		  ...
		  pWnd->GetDlgCtrlID() == IDC_EDIT_total		||
...
void CPro_P452_LinkView::OnChangeEDITVSWR2w() 
{
	UpdateData();
	m_VSWR2 = m_VSWR2w;
	UpdateData(false);	
...
void CPro_P452_LinkView::OnChangeEDITSw() 
{
	UpdateData();
	m_S = m_Sw;
	UpdateData(false);
...
void CPro_P452_LinkView::OnBUTTONSaveProfile() 
{
	CPro_P452_LinkDoc* pDoc = (CPro_P452_LinkDoc *)GetDocument();
...
int CPro_P452_LinkView::DP_LP(CDC* pDC,int nH_DP) //97/10/12
{
...
void CPro_P452_LinkView::initDP_LP(CDC * pDC) //97/10/12
{
//	CDC * pDC = GetDC();
//	pDC->SetMapMode(MM_LOMETRIC);
