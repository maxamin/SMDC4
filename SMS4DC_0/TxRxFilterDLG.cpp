	...
	: CDialog(CTxRxFilterDLG::IDD, pParent)
{
	...
	m_fuFlag = 1;
	m_TxRx = 1;
	m_title ="Transmitter spectrum mask";
	m_num = 20;
}
...
void CTxRxFilterDLG::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_MSFLEXGRID1, m_ListGrid);
	//}}AFX_DATA_MAP
}
...
BOOL CTxRxFilterDLG::OnInitDialog() 
{
	...
	int nCOLS = 3;	
	m_ListGrid.SetCols(nCOLS);
	m_ListGrid.SetColWidth(0,300);
	m_ListGrid.SetColWidth(1,2300);
	m_ListGrid.SetColWidth(2,900);
	m_ListGrid.SetRow(0);
	m_ListGrid.SetCol(1);
	m_ListGrid.SetText("Frequency - Center Frequency");
	m_ListGrid.SetCellAlignment(4);

	m_ListGrid.SetCol(2);
	m_ListGrid.SetText("Atten.(dB)");
	m_ListGrid.SetCellAlignment(4);
...
{
	int yy = MessageBox("Tx filter values are going to be changed.\t\rThe old values will be lost if you choose 'Yes' button.\t\r\rDo you want to save the new values?\t", "Warning", MB_ICONQUESTION | MB_YESNO);
	if(yy==6)
	{
		OnBUTTONDraw();
		...
		CString str;
		if(m_TxRx==1)
			str.Format("SELECT * FROM TxFilters WHERE (((EqID)=%ld)) ORDER BY CS;",m_ID);
		else
			str.Format("SELECT * FROM RxFilters WHERE (((EqID)=%ld)) ORDER BY CS;",m_ID);
