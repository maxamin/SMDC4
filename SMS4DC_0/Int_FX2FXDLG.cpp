// Int_FX2FXDLG.cpp : implementation file
//
...
CInt_FX2FXDLG::CInt_FX2FXDLG(CWnd* pParent /*=NULL*/)
	: CDialog(CInt_FX2FXDLG::IDD, pParent)
{
	//{{AFX_DATA_INIT(CInt_FX2FXDLG)
	m_editX = _T("");
	m_editY = _T("");
	m_rowsY = 0;
	//}}AFX_DATA_INIT
...
void CInt_FX2FXDLG::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CInt_FX2FXDLG)
...
BEGIN_MESSAGE_MAP(CInt_FX2FXDLG, CDialog)
	//{{AFX_MSG_MAP(CInt_FX2FXDLG)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CInt_FX2FXDLG message handlers

BOOL CInt_FX2FXDLG::OnInitDialog() 
{
	...
	m_LIST1.SetCol(5);	m_LIST1.SetText(_Z("Freq(MHz)"));
...
//////////////////////////////////////////
	...
	m_LIST2.SetCol(9);	m_LIST2.SetText(_T("S(dBm)"));
...
void CInt_FX2FXDLG::OnOK() 
{
	...
	if (oFileDlg.DoModal() == IDOK)
	{
		...
		CHTML_Report HtmlReport(FALSE, "OutPut HTML","Behzad Paknahad",_Z("Wanted Hop (Tx): "),_Z("P.452 FX2FX : Interference from the Wanted Hop to the Selected Hop(s)"));
		HtmlReport.InsertTableHeader(iCol, _Z("No."), _Z("TxName"), _Z("TxLocation"), _Z("RxName"), _Z("RxLocation"), _Z("Freq(MHz)"));
		for( j = 1; iRow>1; j++)
		{
		...
		CHTML_Report HtmlReport1(TRUE, "OutPut HTML","Behzad Paknahad",_Z("Selected Hop(s) (Rx): "));
		HtmlReport1.InsertTableHeader(iCol, _Z("No."), _Z("TxName"), _Z("TxLocation"), _Z("RxName"), _Z("RxLocation"), _Z("IntDist(km)"), _Z("PathLoss(dB)"), "I-S(dB)", "I(dBm)", "S(dBm)", _Z("Path Type"));
		for( j = 1; iRow>1; j++)
		{
			ResultSTR.Format("%d",j);
			sCELL[0] = ResultSTR;
