// Pro_P1546_NetBView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPro_P1546_NetBView, CView)

CPro_P1546_NetBView::CPro_P1546_NetBView()
{
	old_cx=old_cy=600;
	new_cx=new_cy=600;
	m_LegendFlag=false;
...
BEGIN_MESSAGE_MAP(CPro_P1546_NetBView, CView)
	//{{AFX_MSG_MAP(CPro_P1546_NetBView)
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1546_NetBView drawing

#include "memdc.h"
void CPro_P1546_NetBView::OnDraw(CDC* pDC)	//98/01/25
{
	CMemDC mDC(pDC);
	OnDraw4(mDC);
}
void CPro_P1546_NetBView::OnDraw4(CDC* pDC)	//98/01/25
{
	CPro_P1546_NetBDoc *pDoc =(CPro_P1546_NetBDoc *) GetDocument();
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1546_NetBView diagnostics

#ifdef _DEBUG
void CPro_P1546_NetBView::AssertValid() const
{
	CView::AssertValid();
}

void CPro_P1546_NetBView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CPro_P1546_NetBView message handlers

void CPro_P1546_NetBView::OnSize(UINT nType, int cx, int cy) 
{
...
void CPro_P1546_NetBView::OnToolsThreshold() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnShowLegend() 
{
...
void CPro_P1546_NetBView::OnUpdateShowLegend(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(m_LegendFlag ? 1 : 0);
}

void CPro_P1546_NetBView::OnTopo0() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnUpdateTopo0(CCmdUI* pCmdUI) 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_0" ? 1 : 0);
}

void CPro_P1546_NetBView::OnTopo1() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnUpdateTopo1(CCmdUI* pCmdUI) 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Topo_1" ? 1 : 0);
}

void CPro_P1546_NetBView::OnJet() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnUpdateJet(CCmdUI* pCmdUI) 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Jet" ? 1 : 0);
}

void CPro_P1546_NetBView::OnGray() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnUpdateGray(CCmdUI* pCmdUI) 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Gray" ? 1 : 0);
}

void CPro_P1546_NetBView::OnPink() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnUpdatePink(CCmdUI* pCmdUI) 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Pink" ? 1 : 0);
}

void CPro_P1546_NetBView::OnCopper() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnUpdateCopper(CCmdUI* pCmdUI) 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Copper" ? 1 : 0);
}

void CPro_P1546_NetBView::OnGlobe() 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
...
void CPro_P1546_NetBView::OnUpdateGlobe(CCmdUI* pCmdUI) 
{
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	pCmdUI->SetCheck(pDoc->colormaptype=="Globe" ? 1 : 0);
}

void CPro_P1546_NetBView::OnMouseMove(UINT nFlags, CPoint point) 
{
	///////////////////// StatusBAR/////////////////////////////////////
	CPro_P1546_NetBDoc* pDoc = (CPro_P1546_NetBDoc*)GetDocument();
	...
	if (pDoc->m_ReadyDoc == 1)
	{
		...
		double lon=pDoc->P1546lon0 + (double)((point.x)*(pDoc->Width-1))*
						(pDoc->m_Resolution_x/(double)(pDoc->m_ZoomFactor))
						/((double)(rect.Width()-1));

		double lat=pDoc->P1546lat0 + (double)((rect.Height()-1 -point.y)*(pDoc->Height-1))*
						(pDoc->m_Resolution_x/(double)(pDoc->m_ZoomFactor))
...
//Monitoring 98/12/17
CString Cov1[3];
if(m_modeFlagP1546NetB == 2)
{
	Cov1[0] = _Z("Listening");
	Cov1[1] = _Z("Measurement");
	Cov1[2] = _Z("Direction-Finding");
}
else if(m_modeFlagP1546NetB == 3)
{
	Cov1[0] = "TDoA";
}

if     (m_modeFlagP1546NetB == 0)	string.Format(_Z("Server :")+" %s",( (E==0) ? _Z("No Station") : pDoc->P1546Name_ST[E-1]) ); 		//Best Server
else if(m_modeFlagP1546NetB == 1)	string.Format(_Z("Server :")+" %s",( (E==0) ? _Z("No Station") : pDoc->P1546Name_ST[E-1]) );		//Monitoring Rx Coverage(BS)
else if(m_modeFlagP1546NetB == 2)	string.Format(_Z("Coverage")+" : %s",( (E==0) ? _Z("No Coverage") : Cov1[E-1]) );					//Monitoring coverage zones
else if(m_modeFlagP1546NetB == 3)	string.Format(_Z("Coverage")+" : %s", ( (E==0) ? _Z("No TDoA") : Cov1[E-1]) );						//Monitoring  TDoA


//		string.Format(_Z("Server :")+" %s",( (E==0) ? _Z("No Station") : pDoc->P1546Name_ST[E-1]) );
...
void CPro_P1546_NetBView::DEG2DMS(CString type,double x,int *xD,int *xM,double *xS,char *xU) 
{
	if (x < 0)
	{
		x=-x;
...
void CPro_P1546_NetBView::LegendLOC(CDC* pDC) 
{
	CPro_P1546_NetBDoc *pDoc =(CPro_P1546_NetBDoc *) GetDocument();

	if     (m_modeFlagP1546NetB == 0)	LegendLOC0(pDC); 		//Best Server
	else if(m_modeFlagP1546NetB == 1)	LegendLOC0(pDC);		//Monitoring Rx Coverage(BS)
	else if(m_modeFlagP1546NetB == 2)	LegendLOC1(pDC);		//Monitoring coverage zones
	else if(m_modeFlagP1546NetB == 3)	LegendLOC2(pDC);		//Monitoring  TDoA
}
...
//		RGBt[i]=m_tile.RGBt[ tt*(i) ];		//98/01/19
	...
	for ( i=0;i<Nrow;i++)
	{
		label.Format("%s",pDoc->P1546Name_ST[Nrow-1-i]);
...
//		RGBt[i]=m_tile.RGBt[ tt*(i) ];		//98/01/19
	...
	pDC->TextOut(x1+5*pscale,y1+5*pscale,_Z("Coverage Zones"));
//	pDC->TextOut(x1+5*pscale,y1+5*pscale,_Z("Best Server"));
	pDC->SetTextAlign(TA_LEFT);

	for ( i=0;i<Nrow;i++)
	{
		label.Format("%s",Cov1[Nrow-1-i]);
//		label.Format("%s",pDoc->P1546Name_ST[Nrow-1-i]);
		pDC->TextOut(x1+w+(5+5)*pscale,y1+(22+(i*tt1+tt1/2))*pscale,label);
	}
	label.Format("%s",_Z("No Coverage"));
//	label.Format("%s",_Z("No Station"));
	pDC->TextOut(x1+w+(5+5)*pscale,y1+(22+(255-tt1/2))*pscale,label);
...
//		RGBt[i]=m_tile.RGBt[ tt*(i) ];		//98/01/19
	...
	pDC->TextOut(x1+5*pscale,y1+5*pscale,_Z("Coverage Zone"));
//	pDC->TextOut(x1+5*pscale,y1+5*pscale,_Z("Coverage zones"));
//	pDC->TextOut(x1+5*pscale,y1+5*pscale,_Z("Best Server"));
	pDC->SetTextAlign(TA_LEFT);
...
void CPro_P1546_NetBView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	m_bLPressed=true;
	if (m_LegendFlag)
	{
...
void CPro_P1546_NetBView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	m_bLPressed=false;
	m_bLSelect=false;
...
BOOL CPro_P1546_NetBView::OnPreparePrinting(CPrintInfo* pInfo) 
{
	return DoPreparePrinting(pInfo);
}

void CPro_P1546_NetBView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
}
void CPro_P1546_NetBView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/) 
{
}
void CPro_P1546_NetBView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	if(pInfo)
	{
...
void CPro_P1546_NetBView::OnFilePrintPreview() 
{
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ToolbarCheck();
	CView::OnFilePrintPreview();
}

void CPro_P1546_NetBView::OnEndPrintPreview(CDC* pDC, CPrintInfo* pInfo, POINT point, CPreviewView* pView) 
{
...
/////////////////////////////Google Earth//////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void CPro_P1546_NetBView::CaptureDC()
{
	CPro_P1546_NetBDoc *pDoc =(CPro_P1546_NetBDoc *) GetDocument();
	...
	if(((CSMS4DCApp *)AfxGetApp())->KmlSave( &sFile, &sPath , &flagShow , fileOUTkml))
	{
		...
		double lonW = pDoc->P1546lon0;
		double lonE = pDoc->P1546lon0 + (double)(pDoc->Width -1)*( pDoc->m_Resolution_x / (double)(pDoc->m_ZoomFactor));
		double latS = pDoc->P1546lat0;
		double latN = pDoc->P1546lat0 + (double)(pDoc->Height-1)*( pDoc->m_Resolution_x / (double)(pDoc->m_ZoomFactor));
...
void CPro_P1546_NetBView::OnToolsGoogle() 
{
	CaptureDC();
}

void CPro_P1546_NetBView::OnFileSaveAs() 
{
	OnFileSave();	
}
void CPro_P1546_NetBView::OnFileSave() 
{
	CPro_P1546_NetBDoc *pDoc =(CPro_P1546_NetBDoc *) GetDocument();
	CString str = pDoc->GetTitle();
...
void CPro_P1546_NetBView::OnInitialUpdate() 
{
	CView::OnInitialUpdate();

	CString Title_En  = "Best Server";
//	CString Title_En  = "Best Server (P1546)";
	CString Title_Num = GetDocument()->GetTitle();		Title_Num.Replace(Title_En,"");
	
//Monitoring	98/12/17
	m_modeFlagP1546NetB = ((CSMS4DCApp *)AfxGetApp())->m_modeFlagP1546NetB;

	if     (m_modeFlagP1546NetB == 1)	Title_En  = "Monitoring Rx Coverage(BS)";	//Monitoring Rx Coverage(BS)
	else if(m_modeFlagP1546NetB == 2)	Title_En  = "Coverage Zones";				//Monitoring coverage zones
	else if(m_modeFlagP1546NetB == 3)	Title_En  = "Location Zone TDoA";			//Monitoring  TDoA
	
	GetParent()->SetWindowText(_Z(Title_En) + Title_Num); 
	((CMainFrame*)(AfxGetApp()->m_pMainWnd))->ChangeMenuLang(IDR_SMS4DCTYPE16);
}

BOOL CPro_P1546_NetBView::OnEraseBkgnd(CDC* pDC) 
{
	return true;	
