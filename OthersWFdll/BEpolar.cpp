...
BOOL CBEpolar::OnInitDialog() 
{
	...
	CString str;
	if(m_Meanflag)	str = m_flagLQ  ?  _Z("Direction-Level (Mean Value)") : _Z("Direction-Quality (Mean Value)");
	else			str = m_flagLQ  ?  _Z("Direction-Level") : _Z("Direction-Quality");
	SetWindowText(str);
...
void CBEpolar::GetFrequency() 
{
	...
	for(long i=0;i<m_F.List.GetSize();i++)
	{
		if(f==m_F.List[i])
		{
			...
			if(m_L1.List.GetSize()>0)		m_L.List.Add(m_L1.List[i]);
			...
			if(m_L1.Index.GetSize()>0)		m_L.Index.Add(m_L1.Index[i]);
		}
	}
