// BEpolar.cpp : implementation file
//
...
CBEpolar::CBEpolar(CWnd* pParent /*=NULL*/)
	: CDialog(CBEpolar::IDD, pParent)
{
	//{{AFX_DATA_INIT(CBEpolar)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CBEpolar::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CBEpolar)
...
BEGIN_MESSAGE_MAP(CBEpolar, CDialog)
	//{{AFX_MSG_MAP(CBEpolar)
	ON_WM_PAINT()
...
/////////////////////////////////////////////////////////////////////////////
// CBEpolar message handlers

BOOL CBEpolar::OnInitDialog() 
{
	CDialog::OnInitDialog();
	m_ThickLines = 1;	
	m_fill = true;
	m_fillcolor = RGB(0,0,70);
	m_5degreestick = 2;
	...
	CString str;
	if(m_Meanflag)	str = m_flagLQ  ?  _T("Direction-Level (Mean Value)") : _T("Direction-Quality (Mean Value)");
	else			str = m_flagLQ  ?  _T("Direction-Level") : _T("Direction-Quality");
	SetWindowText(str);

	GetDlgItem(IDC_RADIO1)->EnableWindow(m_L.flag);
	GetDlgItem(IDC_RADIO2)->EnableWindow(m_Q.flag);
...
//	Set_Static_R();
//	m_BUTTON_Pre.LoadBitmaps(IDB_Pre1,IDB_Pre2,IDB_Pre3,IDB_Pre4);
//	m_BUTTON_First.LoadBitmaps(IDB_First1,IDB_First2,IDB_First3,IDB_First4);
//	m_BUTTON_Next.LoadBitmaps(IDB_Next1,IDB_Next2,IDB_Next3,IDB_Next4);
//	m_BUTTON_Last.LoadBitmaps(IDB_Last1,IDB_Last2,IDB_Last3,IDB_Last4);
	
	CArray<double, double> mFr;
	BOOL flag1;
	double f;
	long i,j;

	mFr.Add(m_F.List[0]);
	for( i=0;i<m_F.List.GetSize();i++)
	{
		flag1 = FALSE;
		f = m_F.List[i];
		for( j=0;j<mFr.GetSize();j++)
		{
			if(f==mFr[j]) { flag1 = FALSE;	break;}
			else			flag1 = TRUE;
		}
		if(flag1)	
		{
	...
	GetFrequency();
	m_D.max = m_D1.max;		m_Q.max = m_Q1.max;		m_L.max = m_L1.max;
	m_D.min = m_D1.min;		m_Q.min = m_Q1.min;		m_L.min = m_L1.min;
	m_D.flag = m_D1.flag;	m_Q.flag = m_Q1.flag;	m_L.flag = m_L1.flag;
	Set_Static_R();

	return TRUE;  
}

void CBEpolar::GetFrequency() 
{
	CString str;
	long n = m_COMBO_Fr.GetCurSel();
	m_COMBO_Fr.GetLBText(n,str);
	double f = atof(str);
	...
	for(long i=0;i<m_F.List.GetSize();i++)
	{
		if(f==m_F.List[i])
		{
			m_D.List.Add(m_D1.List[i]);
			m_Q.List.Add(m_Q1.List[i]);
			m_L.List.Add(m_L1.List[i]);
			m_ListDateTime.Add(m_ListDateTime1[i]);
			str = m_ListDateTime1[i];
			str.Replace("_","    ");
			m_COMBO_Time.AddString(str);

			m_D.Index.Add(m_D1.Index[i]);
			m_Q.Index.Add(m_Q1.Index[i]);
			m_L.Index.Add(m_L1.Index[i]);
		}
	}
...
void CBEpolar::DrawPattern(int HV,int x0,int y0,int radius,int offset) 
{
	CClientDC dc(this);
	CString str;
...
///////////////////////////////////////////////////////////////////////////////////////
	...
	long N = m_D.List.GetSize();
	double v , vMax , vMin;
	int c , X1 , Y1;

	if(m_flagLQ)	{vMax = m_L.max;	vMin = m_L.min;}
	else			{vMax = m_Q.max;	vMin = m_Q.min;}


	if(m_single)
	{
		if(m_flagLQ)	v = m_L.List[m_i];
		else			v = m_Q.List[m_i];
		a = (m_D.List[m_i])*pi/180.0;
	...
	else
	{
		for (i=1;i<N;i++)
		{
			if(m_flagLQ)	v = m_L.List[i];
			else			v = m_Q.List[i];
			a = (m_D.List[i])*pi/180.0;
...
void CBEpolar::DrawEllipse(BOOL nMode,int x1,int y1,int x2,int y2,int nStyle,int nWidth,COLORREF pColor,int x3,int y3,int x4,int y4,COLORREF bColor) 
{
	CClientDC dc(this);
	CBrush brush(bColor);
...
void CBEpolar::DrawLine(int x1,int y1,int x2,int y2,int nStyle,int nWidth,COLORREF pColor) 
{
	CClientDC dc(this);
	CPen pen(nStyle,nWidth,pColor);
	CPen* OLDpen = dc.SelectObject(&pen);
...
int CBEpolar::Contour(double y,double MinV,double MaxV,int nL) 
{
	if (nL>=255)
...
void CBEpolar::ColorMap(CString type)
{
	if (type=="Topo_1")
	{
		RGBt[  0] = RGB(      0,      0,      150);
...
void CBEpolar::Set_Static_R() 
{
	CString str;
	str.Format(_T("%ld of %ld") , m_i+1 , m_D.List.GetSize()-0);
	GetDlgItem(IDC_STATIC_R)->SetWindowText(str);

	m_ListDateTime[m_i].Replace(_T("_"),_T("    "));
	str.Format(_T("%g") , m_D.List[m_i]);	GetDlgItem(IDC_STATIC_D )->SetWindowText(str);

	if(m_L.flag)	str.Format(_T("%g") , m_L.List[m_i]);
	else			str.Format(_T("") );
	GetDlgItem(IDC_STATIC_L )->SetWindowText(str);

	if(m_Q.flag)	str.Format(_T("%g") , m_Q.List[m_i]);
	else			str.Format(_T("") );
	GetDlgItem(IDC_STATIC_Q )->SetWindowText(str);

	if(m_Lu == _T("dBµV/m"))	m_Lu = _T("dBuV/m");
	else if(m_Lu == _T("dBµV"))	m_Lu = _T("dBuV");

	str.Format(_T("Level(%s)") , m_Lu);	GetDlgItem(IDC_STATIC_L0)->SetWindowText(str);
}

HBRUSH CBEpolar::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
...
void CBEpolar::OnSelchangeCOMBOFr() 
{
	GetFrequency();	
	Set_Static_R();
	Invalidate();
...
void CBEpolar::OnSelchangeCOMBOTime() 
{
